...
public class AppointmentController implements Initializable {
   ...
   // DatabaseManager appointedDoctors;
    ...
    try {
        return Appointments.addAppointment(appointment);
    } catch (Exception e) {
        e.printStackTrace();
        return false;
    }
        ...
        if (validateOtherBookingForm()) {
            try {
                ...
                if (patientName != null && !patientName.isEmpty() && 
                    ...
                    boolean addedToList = Appointments.addAppointment(appointment);

                    if (addedToList) {
                        boolean savedToDatabase = saveAppointmentToDatabase(appointment);
                        ...
                        if (savedToDatabase) {
                            ...
                            showErrorAlert("Lỗi cơ sở dữ liệu", "Không thể lưu lịch hẹn vào cơ sở dữ liệu.");
                        }
                    } else {
